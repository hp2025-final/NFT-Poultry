...
@section('title', 'Product Categories - NF Dev')
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
            <form method="GET" action="{{ route('categories.index') }}" class="d-flex gap-2 flex-grow-1" style="max-width: 400px;">
                <input type="text" name="q" class="form-control" value="{{ request('q') }}" placeholder="Search product categories...">
                <button type="submit" class="btn btn-primary">Search</button>
            </form>
            <div class="btn-group shadow-sm">
                <a href="{{ route('categories.index', ['show' => 'active']) }}" class="btn btn-outline-primary {{ $show === 'active' ? 'active' : '' }}">Active</a>
                <a href="{{ route('categories.index', ['show' => 'archived']) }}" class="btn btn-outline-primary {{ $show === 'archived' ? 'active' : '' }}">Archived</a>
            </div>
        </div>
    </div>
</div>
...
<div class="row g-4">
            ...
            <div class="card-body d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-sm">
                        <li><a class="dropdown-item" href="{{ route('categories.edit', $category->id) }}"><i class="bi bi-pencil me-2"></i>Edit</a></li>
    ...
    @empty
    <div class="col-12 text-center py-5 text-muted">No product categories found.</div>
    @endforelse
</div>
