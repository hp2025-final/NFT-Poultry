...
@section('title', $customer->name . ' - Customer Ledger - NF Dev')

@section('content')
<div class="row align-items-center mb-4">
    <div class="col-sm-6">
        <h2 class="mb-0">{{ $customer->name }}</h2>
        <p class="text-muted mb-0">Customer Account Ledger</p>
    </div>
    <div class="col-sm-6 text-sm-end mt-3 mt-sm-0">
        <div class="btn-group shadow-sm">
            <a href="{{ route('reports.customers_a4', array_merge(['id' => $customer->id], request()->only('start_date', 'end_date'))) }}" target="_blank" class="btn btn-outline-primary">
                <i class="bi bi-file-earmark-pdf me-1"></i>A4 Ledger
            </a>
            <a href="{{ route('reports.customers_thermal', array_merge(['id' => $customer->id], request()->only('start_date', 'end_date'))) }}" target="_blank" class="btn btn-outline-primary">
                <i class="bi bi-printer me-1"></i>Thermal Ledger
            </a>
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('customers.show', $customer->id) }}" class="row g-3 align-items-end">
            <div class="col-md-4">
...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm p-3 text-center {{ $runningBalance > 0 ? 'bg-danger-subtle text-danger' : 'bg-success-subtle text-success' }}">
            ...
            <small class="fw-bold">{{ $runningBalance > 0 ? 'Receivable' : ($runningBalance < 0 ? 'Payable' : 'Clear') }}</small>
        </div>
    </div>
</div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge {{ str_contains($txn['type'], 'Sale') ? 'bg-primary-subtle text-primary' : 'bg-success-subtle text-success' }} border">
                                {{ $txn['type'] }}
                            </span>
                        </td>
                        <td>
